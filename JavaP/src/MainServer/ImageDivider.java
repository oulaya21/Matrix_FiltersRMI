...
public class ImageDivider {
    ...
    public static ArrayList<SubMatrix> divide(int[][] image, int numSlaves) {
        if (numSlaves <= 0) {
            throw new IllegalArgumentException("Number of slaves must be greater than zero.");
        }
        int rows = image.length;
        int cols = image[0].length;
        int rowsPerSlave = rows / numSlaves;  // Division by zero can occur if numSlaves is zero
        int remainingRows = rows % numSlaves;
        ...
        for (int i = 0; i < numSlaves; i++) {
            int endRow = startRow + rowsPerSlave;
            if (remainingRows > 0) {
                endRow++;
                remainingRows--;
            }
